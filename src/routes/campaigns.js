...
router.post(
  ...
  [
    ...
    body('targetLabels').optional().isArray({ max: 100 }).withMessage('targetLabels must be an array with max 100 entries'),
    body('targetLabels.*').optional().isString().trim().isLength({ max: 50 }).withMessage('target label is too long'),
    body('targetJids').optional().isArray({ max: 10000 }).withMessage('targetJids must be an array with max 10000 entries'),
